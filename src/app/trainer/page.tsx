...
// nerdamer is removed from here for security!
...
type Problem = {
  ...
  // Note: Answers are no longer in this type because the API shouldn't send them!
};
...
function TrainerContent() {
  ...
  useEffect(() => {
    ...
    // 1. Get Problems (Now secure: API should NOT return answers)
    fetch("/api/integrals")
      .then((res) => res.json())
      .then((data) => {
        if (!mounted) return;
  ...
  const checkAnswer = async () => {
    ...
    try {
      ...
      if (data.error) {
         setFeedback({ type: "error", msg: data.error === "Syntax Error" ? "Check Syntax" : "Error" });
         triggerShake();
         return;
      }
      ...
      if (data.isCorrect) {
        ...
        if (session) {
           ...
           await fetch("/api/progress", {
             method: "POST",
             headers: { "Content-Type": "application/json" },
             body: JSON.stringify({ problemId: activeProblem.id, isCorrect: true }),
           });
        }
      } else {
        // WRONG
        ...
        // Optional: You could record wrong attempts here if you wanted
      }
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              <div className={`relative ${isShaking ? "animate-shake" : ""}`}>
                <input
                  ...
                  className={`w-full border-2 p-6 pr-32 rounded-3xl text-2xl font-mono text-white bg-white/5 focus:outline-none transition-all ${
